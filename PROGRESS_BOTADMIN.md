...
| ۲۳ — تیکت‌های بات | ✅ | **جدید:** `routes/botSupportTickets.ts`، `tickets/TicketsSection.tsx`. **تغییر:** `routes/index.ts`، `BotWorkspaceDocument.tsx` (unlock)، ۵ locale (`botTickets`). | هر سه گیت سبز. | `botTickets.ts` قدیمی **دست‌نخورده ماند** — آن قرارداد support-bot پلتفرم است، نه تیکت‌های داخل بات. فایل جدید اسم متفاوتی دارد تا دوباره اشتباه نشود. |
| ۲۴ — یکپارچگی نهایی و مستندسازی | ✅ | **جدید:** `routes/botHealth.ts`، `BotHealthCard.tsx`، `docs/BOT_ADMIN_ON_WEB.md`. **تغییر:** `BotWorkspaceDocument.tsx` (کارت سلامت در overview)، ۵ locale (`botHealth`). | هر سه گیت سبز، ۳۴ تست سبز. | تنها سکشن قفل‌مانده `discounts` است، با دلیل صریح در کد و در سند. |
...
## گزارش فازهای ۲۳ و ۲۴ — تیکت‌ها و یکپارچگی نهایی

### فاز ۲۳ — تیکت‌های داخل بات

**`routes/botTickets.ts` موجود دست‌نخورده ماند.** بررسی‌اش نشان داد اصلاً همان
چیزی نیست که این فاز می‌خواهد: آن قرارداد **support-bot پلتفرم** است — جداول
`tickets`/`ticket_messages` روی Postgres **سایت**، با یک API key مشترک، برای
تیکت‌های خودِ مشتری با IrForge. چیزی که این فاز می‌خواست، تیکت‌هایی است که
کاربرِ بات از داخل خودِ بات باز می‌کند و در تب‌های `tickets`/`ticket_messages`
شیت **تننت** ذخیره می‌شوند (`handlers/support.py`).

دو چیز کاملاً متفاوت با اسم یکسان — همان الگویی که B13 و B14 هم داشتند. برای
همین فایل جدید عمداً اسم متفاوتی دارد (`botSupportTickets.ts`) و کامنت بالای هر
دو تفاوت را صریح می‌گوید، تا کسی دوباره یکی را جای دیگری نگیرد.

شکل و پنج وضعیت (`open`/`assigned`/`closed`/`reopened`/`escalated`) عیناً از
`handlers/support.py` آمده‌اند. تب‌ها lazy از `plugin_db` ساخته می‌شوند، پس
نبودشان روی یک بات **حالت عادی** است (لیست خالی)، نه خطا.

**ترتیب پاسخ:** پیام اول در تب ثبت و بعد به کاربر فرستاده می‌شود؛ تیکتی که
پاسخش ثبت شده ولی نرسیده، بهتر از پاسخی است که رفته و هیچ ردی در تاریخچه ندارد.
UI هر دو حالت را جدا نشان می‌دهد. پاسخ روی تیکت بسته، وضعیت را به `reopened`
برمی‌گرداند (بسته‌بودن با پاسخ ادمین معنا ندارد)، و `message_thread_id` تیکت —
اگر تننت forum-دار باشد — در ارسال رعایت می‌شود تا پاسخ در همان تاپیک بماند.
ارجاع به کسی که ادمین این بات نیست رد می‌شود؛ یک ارجاع مرده بدتر از نبود ارجاع است.

### فاز ۲۴ — سلامت بات و مستندسازی

**`GET /api/bots/:botId/health`** خروجی `panelHealth` فاز ۶ را می‌گیرد و چهار
دسته چک دیگر رویش سوار می‌کند. معیار انتخابشان یک چیز بود: **شکست‌های بی‌صدا** —
چیزهایی که بات درباره‌شان خطا نمی‌دهد، فقط کار نمی‌کند:

| چک | چرا بی‌صداست |
|---|---|
| فرم بدون فیلد | کاربر واردش می‌شود و هیچ سؤالی نمی‌بیند |
| فرم فعال بدون مقصد | پاسخ‌ها ثبت می‌شوند ولی جایی فرستاده نمی‌شوند |
| کامند با مقصد معلق | فقط «پیدا نشد» می‌گیرد |
| پلاگین فعال بدون تب لازم | تا اولین استفاده معلوم نمی‌شود |
| پیام خوش‌آمد خالی | اولین برخورد کاربر با بات، خالی است |
| عضویت اجباری بدون پیام | کاربر نمی‌فهمد چرا نمی‌تواند ادامه دهد |
| پنل فروش بدون اطلاعات پرداخت | کاربر نمی‌داند پول را کجا بفرستد |
| حالت تعمیرِ روشن‌مانده | بات فقط پیام تعمیر می‌دهد و کسی یادش نیست |

هر مورد `section` دارد و دکمه‌ی «رفع» کاربر را دقیقاً همان‌جا می‌برد؛ موارد
`repairable` با یک کلیک روی `/panels/repair` درست می‌شوند. کارت در نمای کلی
هر بات نشسته و برای باتِ بدون شیت **ساکت می‌ماند** به‌جای اینکه خطای قرمز بگذارد.
پاسخ وضعیت زیرساخت (cache-bust، صف، پرچم‌های cutover) را هم برمی‌گرداند.

**`docs/BOT_ADMIN_ON_WEB.md`** نوشته شد: نگاشت کامل «قابلیت ↔ تب ↔ اندپوینت ↔
کامپوننت»، توضیح سه‌لایه‌ی کش و اینکه چرا تأخیر کاملاً صفر نمی‌شود، پرچم‌های
cutover، متغیرهای محیطی، جدول ۱۴ باگ بات و راه‌حلشان، و فهرست چیزهایی که عمداً
فقط در بات ماندند.

### تنها سکشن قفل‌مانده — با دلیل

`discounts` هنوز قفل است. «تخفیف» اینجا **دو معنی** دارد: کدهای تخفیف پلتفرم
(`routes/discounts.ts`، Postgres سایت، فاز ۹ سایت) و پلاگین `discount` بات با
تب `discounts` خودش. وصل‌کردن این سکشن به هرکدام بدون تصمیم صریح، دقیقاً همان
اشتباه B13/B14 را تکرار می‌کرد — و هیچ‌کدام از ۲۴ فاز این تسک پوششش نمی‌دهد.
دلیلش هم در کامنت کد نوشته شده و هم در بخش ۷ سند.

---

## وضعیت نهایی

**همه‌ی ۲۴ فاز تمام شدند.** گیت بیلد بخش ۱.۶ در پایان هر فاز اجرا شد و در پایان
کار هر سه سبزند، به‌علاوه‌ی **۳۴ تست سرور**:

```
pnpm --filter @workspace/api-server run build      ✅ (فقط وارنینگ از قبل موجودِ import.meta)
pnpm --filter @workspace/irforge run build         ✅ ۶۵ صفحه
pnpm --filter @workspace/irforge run typecheck     ✅ صفر خطا
pnpm --filter @workspace/api-server run test       ✅ ۳۴/۳۴
```

**یادداشت صادقانه درباره‌ی تست:** آنچه خودکار تست شده، منطق دامنه است —
حفظ کلیدهای لمس‌نشده، استراتژی‌های حذف، round-trip چیدمان دکمه‌ها، هم‌ارزی
کلاینت/سرور، و round-trip بک‌آپ. مسیرهای HTTP و UI با build و typecheck پوشش
