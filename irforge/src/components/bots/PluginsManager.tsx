/**
 * PluginsManager.tsx — بخش پلاگین‌های یک بات، در دو دسته.
 *
 ...
 * در تب `bot_settings` است. دو مفهوم کاملاً جدا با یک برچسب «فعال». حالا هر
 * کارت **هر دو** را نشان می‌دهد: سوییچ فعال/غیرفعال که روی خود بات اثر
 * می‌گذارد، و بجِ جدای وضعیت خرید.
 *
 * دسته‌بندی (خواسته‌ی جدید): بالا پلاگین‌هایی که روی *این* بات قابل استفاده‌اند
 * (خریده‌شده یا رایگان) با سوییچ و میان‌بر به بخش مدیریتشان؛ پایین پلاگین‌هایی
 * که نداری — با انتخابگرِ «برای کدام بات؟» که پیش‌فرضش همین بات است. به‌محض
 * خرید، پلاگین از پایین می‌رود و بالا ظاهر می‌شود.
 *
 * فهرست پایین قبلاً `item.name`/`item.description` خام مارکت‌پلیس را نشان می‌داد
 * (همیشه انگلیسی)؛ حالا از `PluginLibrary` می‌آید که متن را به زبان کاربر
 * می‌دهد.
 */
...
export function PluginsManager({ botId }: { botId: string }) {
  ...
  // بالا فقط چیزهایی که روی همین بات واقعاً قابل استفاده‌اند. پلاگینِ پول‌دارِ
  // نخریده جایش پایین است، وگرنه همان فهرست قاطیِ قبلی می‌شود.
  const usable = data.plugins.filter((p) => p.purchased || p.isFree);
  ...
  return (
    <div className="space-y-8">
            ...
            {usable.map((plugin) => (
              <Card key={plugin.id}>
                <CardHeader className="pb-3">
                  ...
                  <CardDescription>{pluginDescription(plugin, lang)}</CardDescription>
                </CardHeader>
      ...
      {/* پایین: نداشته‌ها (و اگر روی بات دیگری داری، دکمه‌ی انتقال به این بات). */}
      <PluginLibrary scopeBotId={botId} only="available" />
    </div>
  );
}
