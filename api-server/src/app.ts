...
// ─── POST /api/admin/bots ─────────────────────────────────────────────────────
// Super-admin manually registers a bot by pasting its Telegram token — for
// bots created outside the normal payment flow (support cases, migrations,
// gifted bots, etc). Mirrors approve-payment: grabs a free sheet from the
// pool (if one exists and assignSheet !== false) and generates an admin
// code, so the bot is immediately usable without a fake payment record.

router.post("/admin/bots", requireSuperAdmin, async (req: any, res) => {
  try {
    const { name, token, ownerId, description, assignSheet } = req.body;
    ...
    // Tokens are 1:1 with a Telegram bot — refuse to register the same one twice.
    const existingBots = await db.select().from(botsTable);
    const tokenTaken = existingBots.some((b) => {
      try {
    ...
    if (assignSheet !== false) {
      ...
      if (freeSheet) {
        await db
          ...
          .set({ status: "assigned", assignedBotId: botId })
          .where(eq(sheetPoolTable.id, freeSheet.id));
        syncSheetPoolUpsert({ sheet_id: freeSheet.sheetId, assigned_to: botId, status: "assigned" });
        sheetId = freeSheet.sheetId;
    ...
    syncTenantUpsert({
      ...
      sheet_id: sheetId,
      admin_password: adminCode,
      status: sheetId ? "active" : "inactive",
    ...
    await db.insert(activityTable).values({
      ...
      description: `Bot "${name}" registered by a super admin`,
      botName: name,
    });

    res.status(201).json({ ...formatBot(bot), owner: { id: owner.id, name: owner.name, email: owner.email } });
  } catch (err) {
...
// Super-admin can remove ANY bot on the platform, not just their own — reuses
// the same full-purge logic (Postgres rows + registry/sheet-pool/deletion-
// queue syncs) as the owner-facing DELETE /api/bots/:botId.
