 ...
 * body (e.g. sms.ir's x-api-key auth and template-based Verify API — see
 * lib/smsir.ts, the dedicated adapter that already exists for exactly that
 * case) can add a real per-vendor adapter without touching any caller of
 * sendSms.
 *
...
export interface SmsMessage {
  ...
  text: string;
  /**
   * The raw OTP code, when this message *is* one — a template-based
   * provider (see lib/smsir.ts) needs just this, never the composed
   * sentence in `text`. Optional because most callers, and every send
   * through this generic gateway, only ever have `text`.
   */
  code?: string;
}
